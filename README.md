...
当前源码与最新测试版为 [`v0.2.0-beta.9`](https://github.com/HuaWeiQiu/AgentDeck/releases/tag/v0.2.0-beta.9)，按 ARM64/x86_64 拆分 Secure/Lab APK。ARM64 已完成 Android 16 真机首次 Runtime 准备（含国内 apt 源）及 Secure 远程 MCP 发现、审批、调用与恢复验收；x86_64 已完成构建和包内容校验，但实际 Runtime 启动仍待 x86_64 Android 虚拟设备验证。Beta APK 使用测试签名，不是正式签名的稳定版；构建本身启用 R8、资源压缩和依赖 Baseline Profile。`v0.1.0` 是早期骨架。
...
- 对话可从系统文件选择器添加图片或文件：PNG/JPEG 使用模型原生图片输入；文本/代码、PDF、DOCX、XLSX 在内嵌 Runtime 中转换为受限纯文本副本后交给 Codex，不执行宏、公式或脚本；单次最多 4 个，单个最多 20 MiB。
- app-server 仅监听回环地址，每次启动生成一次性 capability token，并限制消息大小与连接生命周期。
- ARM64 和 x86_64 候选构建可一键下载并校验对应架构的 Ubuntu Base 24.04.4 与 Codex CLI 0.147.0，安全解包、验证后原子启用；APK 不跨架构混装 Runtime 组件。
...
- 设置中的“扩展”按需管理 Skill 与远程 HTTPS MCP；对话只选择自己需要的扩展。发现后的工具在固定高度区域逐项显示并可独立滚动，MCP 工具默认逐次审批，工具开关形成 allowlist，Bearer 进入独立 Android Keystore，不进入 Runtime 环境变量。
...
完整决策见 [三级体验](docs/ADR-0008-CUSTOMER-EXPERIENCE-MODES.md)、[内嵌本地 Runtime](docs/ADR-0009-EMBEDDED-LOCAL-RUNTIME.md) 和 [受管扩展](docs/ADR-0013-MANAGED-EXTENSIONS.md)。`0.2.0-beta.9` 已在一台 Android 16 ARM64 真机上通过聊天性能第二阶段（50/300/1000-turn PSS 平台化）与 Runtime 下载看门狗验收；Lab 共享同一聊天栈，本轮未单独做真机采集。
...
转为稳定版前仍需按 [真机发布清单](docs/RELEASE_CHECKLIST.md) 完成完整验收；自动化测试不能替代 Android、内嵌 PRoot 和真实网络的端到端验证。
...
第一条命令会检查配方与 APK assets、wrapper 同步，运行 JVM 单测，构建启用 R8 的测试签名双 ABI Beta APK，并执行 Android Lint。第二条运行 RPC、Runtime 下载/标记和文件适配异常矩阵，并生成不含聊天正文或凭据的 JSON 报告。
...
- `android/app/build/outputs/apk/secure/beta/app-secure-arm64-v8a-beta.apk`
- `android/app/build/outputs/apk/secure/beta/app-secure-x86_64-beta.apk`
- `android/app/build/outputs/apk/lab/beta/app-lab-arm64-v8a-beta.apk`
- `android/app/build/outputs/apk/lab/beta/app-lab-x86_64-beta.apk`
- `android/app/build/reports/lint-results-secureBeta.html`
- `android/app/build/reports/lint-results-labBeta.html`
- `android/app/build/reports/stability/host-matrix.json`
...
- 当前源码与测试预发布：[v0.2.0-beta.9](https://github.com/HuaWeiQiu/AgentDeck/releases/tag/v0.2.0-beta.9)
