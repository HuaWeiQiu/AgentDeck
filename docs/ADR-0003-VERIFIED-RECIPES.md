...
5. Ubuntu 新安装固定为 `ubuntu:24.04`。如果本地同名容器版本不同，配方明确失败并要求用户迁移，不自动删除已有数据。
6. Codex 固定为 OpenAI 官方 `0.147.0` 独立 Linux 二进制。arm64 和 x86_64 下载分别校验 GitHub Release 记录的 SHA-256，再写入 `/usr/local/bin/codex`。
7. Codex wrapper 由配方声明 assets 名称，安装器从 APK 读取后写入 Termux；文件名和 heredoc 边界均校验，权限固定为 `0700`。
8. Claude Code 继续显示为 P1 规划项，但在完成独立 adapter 和真实验证前不提供安装按钮。
